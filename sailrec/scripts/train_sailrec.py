"""Train SAILRec — SASRec + InfoNCE alignment to frozen LLM embeddings.
...
from sailrec.training.sailrec_trainer import SAILRecTrainer
...
def prepare_data(cfg: SAILRecConfig) -> tuple:
    """Load + split + cache negatives. See ``train_p1.prepare_data``."""
    data_dir = "data"
...
def train_single_seed(
    ...
    """Train SAILRec for one (seed, λ) combo. Returns sampled test metrics.
    ...
    if wandb_disabled:
        ...
        try:
            ...
            wandb_run = wandb.init(
                ...
                name=f"sailrec_lambda{cfg.align.lambda_align}_seed{seed}",
                config={
                    "seed": seed,
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Train SAILRec (SASRec + item-level InfoNCE alignment to "
    ...
    parser.add_argument(
        ...
        help="SAILRec experiment config.",
    )
    parser.add_argument(
        "--base-config",
    ...
    log(f"SAILRec | seeds={cfg.seeds} | device={cfg.device}")
    log(
    ...
    if args.llm_init:
        ...
        # dependency. ``train_a1_llm_init.pca_project`` is the same
        ...
        from train_a1_llm_init import pca_project
