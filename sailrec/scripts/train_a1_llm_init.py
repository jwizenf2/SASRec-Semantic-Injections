...
Identical to ``train_p1.py`` (vanilla SASRec, BCE-only training,
...
::
    ...
    sailrec/outputs/ablations/A1_llm_init/
    ├── neg_samples.npz
...
Same shape as ``train_p1.py``'s output so ``results_table.py`` and
...
# Reuse train_p1's data prep — it produces the same split + neg samples
# A1 needs.
sys.path.insert(0, str(Path(__file__).resolve().parent))
from train_p1 import log, prepare_data  # noqa: E402
...
# ---------------------------------------------------------------------------
# Single-seed training — mirrors train_p1.train_single_seed
# ---------------------------------------------------------------------------
...
def train_single_seed(
    ...
    # Optional W&B (same gate as train_p1).
    import os
...
def main():
    ...
    parser.add_argument(
        "--config",
        default="sailrec/configs/ablations/A1_llm_init.yaml",
    ...
    # ``llm_embeddings_path`` field that P1Config doesn't carry.
    import yaml
    ...
    # Aggregate identical to train_p1.
    log(f"\n{'='*60}\nAGGREGATE\n{'='*60}")
