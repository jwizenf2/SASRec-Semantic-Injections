...
:func:`sailrec.data.movielens.preprocess` and
:func:`sailrec.data.movielens.build_user_sequences`, which are
dataset-agnostic but live in the MovieLens module for historical
reasons. Most callers can ``from sailrec.data.loaders import …`` and
...
from sailrec.data.movielens import build_user_sequences, preprocess
...
def resolve_amazon_category(dataset: str) -> str:
    ...
    Returns:
        Bare category name accepted by :func:`sailrec.data.amazon.load_ratings`,
        e.g. ``"Video_Games"``.
...
def load_ratings(data_dir: str | Path, dataset: str) -> pd.DataFrame:
    ...
    if is_movielens(dataset):
        from sailrec.data.movielens import load_ratings as _ml_load
        return _ml_load(data_dir, dataset=dataset)

    if is_amazon(dataset):
        from sailrec.data.amazon import load_ratings as _amz_load
        return _amz_load(data_dir, dataset=resolve_amazon_category(dataset))

    if is_yelp(dataset):
        from sailrec.data.yelp import load_ratings as _yelp_load
        return _yelp_load(data_dir, dataset=dataset)

    if is_amazon2018(dataset):
        from sailrec.data.amazon2018 import load_amazon2018
