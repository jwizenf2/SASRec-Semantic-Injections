"""BERT4Rec trainers — Cloze-task baseline and SAILRec variant.
...
* :class:`BERT4RecSAILTrainer`  — A8 equivalent for BERT4Rec. Adds the
                                  SAILRec InfoNCE alignment loss on top of
                                  the Cloze loss.

Both subclass :class:`sailrec.training.trainer.BaseTrainer` and override
...
:class:`~sailrec.data.dataset.SASRecTrainDataset`), BERT4Rec trainers
expect ``(masked_seq, labels)`` pairs from
:class:`~sailrec.data.dataset.BERT4RecTrainDataset`:
...
SAILRec alignment (BERT4RecSAILTrainer only)
--------------------------------------------

The InfoNCE alignment loss is identical to SAILRecTrainer: look up the unique
...
from sailrec.alignment.contrastive import (
    AlignProjector,
    infonce_align_loss,
...
class BERT4RecSAILTrainer(BaseTrainer):
    ...
    Combines the BERT4Rec Cloze objective with the SAILRec item-table
    InfoNCE alignment loss. The alignment component is identical to
    :class:`~sailrec.training.sailrec_trainer.SAILRecTrainer`: a frozen
    ...
    def __init__(
        ...
        # Frequency-weighted item weights — same formula as SAILRecTrainer.
