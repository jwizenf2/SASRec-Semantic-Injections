"""Training primitives shared across P1 and SAILRec.
...
The corresponding SAILRec trainer
(:class:`sailrec.training.sailrec_trainer.SAILRecTrainer`) is in a
...
class BaseTrainer:
    """Concrete training loop shared by every SAILRec / P1 trainer.
    ...
    * :meth:`save_checkpoint` /
      :meth:`load_checkpoint`     — when extra trainer state needs to
                                    be persisted (the SAILRec trainer
    ...
    def __init__(
        ...
        # that own extra trainable modules (e.g. SAILRec's
    ...
    def _summary_config(self) -> dict[str, Any]:
        ...
        ``lambda_align`` and ``temperature`` for SAILRec).
        """
        return {
            "lr": self.lr,
    ...
    # AlignProjector (in SAILRec), early-stopping bookkeeping, every
    ...
    def _extra_resume_state(self) -> dict[str, Any]:
        ...
        SAILRecTrainer overrides this to save its AlignProjector. The
...
class SASRecTrainer(BaseTrainer):
    ...
    the SAILRec trainer in the sibling module.
    """
    ...
    def save_checkpoint(self, path: str | Path) -> None:
        ...
        modern default used by SAILRec. ``BaseTrainer.load_checkpoint``
        accepts both.
        """
