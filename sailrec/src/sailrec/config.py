"""YAML → dataclass configuration loaders for SAILRec experiments.

Two configs in this module:

* :class:`P1Config`       — vanilla SASRec (used by ``train_p1.py`` and
                            by ``eval_fullrank.py``).
* :class:`SAILRecConfig`  — SAILRec method (used by ``train_sailrec.py``).
...
4. Add a per-experiment yaml under ``sailrec/configs/``.
...
# ---------------------------------------------------------------------------
# Sub-configs (shared across P1 and SAILRec)
# ---------------------------------------------------------------------------
...
class SASRecConfig:
    ...
    These are intentionally identical between P1 and SAILRec — SAILRec
...
class P1Config:
    ...
    wandb_project: str = "sailrec"
    wandb_entity: str | None = None

    @classmethod
    def from_dict(cls, d: dict) -> "P1Config":
        """Build a P1Config from a plain dict (typically from YAML)."""
...
# ---------------------------------------------------------------------------
# SAILRec: vanilla SASRec + InfoNCE alignment
# ---------------------------------------------------------------------------


@dataclass
class ContrastiveAlignConfig:
    """SAILRec auxiliary-loss hyperparameters.
    ...
    embeddings_path: str = "sailrec/outputs/embeddings/video_games.pt"
...
@dataclass
class SAILRecConfig:
    """Top-level config for SAILRec training.

    Identical schema to :class:`P1Config` plus an ``align`` block that
    holds the auxiliary-loss hyperparameters. The architecture (``model``)
    is unchanged: SAILRec does not modify SASRec.
    """
    ...
    @classmethod
    def from_dict(cls, d: dict) -> "SAILRecConfig":
        """Build a SAILRecConfig from a plain dict (typically from YAML)."""
        model_cfg = SASRecConfig(**d.get("model", {}))
        train_cfg = TrainingConfig(**d.get("training", {}))
        eval_cfg = EvalConfig(**d.get("evaluation", {}))
        ...
        return cls(
            ...
            seeds=d.get("seeds", [42]),
            device=d.get("device", "mps"),
            output_dir=d.get("output_dir", "sailrec/outputs/sailrec"),
            dataset_name=dataset.get("name", "amazon-Video_Games"),
            min_interactions=dataset.get("min_interactions", 5),
            wandb_project=tracking.get("wandb_project", "sailrec"),
            wandb_entity=tracking.get("wandb_entity"),
