"""Yelp Open Dataset loader for ItemTable.

Counterpart to :mod:`sasrec_injection.data.amazon` and
:mod:`sasrec_injection.data.movielens`. Ingests the public Yelp Open Dataset
...
which makes it the only dataset where ItemTable can be put in a
...
What this loader produces is *5-core ItemTable-style* preprocessing,
...
* Leave-one-out split (handled downstream in
  :func:`sasrec_injection.data.splitting.leave_one_out_split`).
...
# :func:`sasrec_injection.data.loaders.load_ratings`. The bare string ``"yelp"``
...
def load_ratings(data_dir: str | Path, dataset: str = DATASET_NAME) -> pd.DataFrame:
    ...
    Args:
        ...
        dataset: Always ``"yelp"`` here. Accepted as a kwarg for API
            symmetry with :func:`sasrec_injection.data.amazon.load_ratings` and
            :func:`sasrec_injection.data.movielens.load_ratings`; ignored.
    ...
    Returns:
        ...
        IDs); :func:`sasrec_injection.data.movielens.preprocess` will remap
