...
ItemTable uses this exact architecture; the auxiliary alignment loss
lives entirely on the trainer (see
:class:`sasrec_injection.training.item_table_trainer.ItemTableTrainer`). Saved
...
training uses every position via BCE; ItemTable inference uses only the
last position (``x[:, -1, :]``).
...
class SASRec(nn.Module):
    ...
    * ``llm_item_emb`` + ``fusion_mode="add"`` — A2 Input-Fusion
    ...
    def __init__(
        ...
        # ItemTable alignment loss reshapes.
