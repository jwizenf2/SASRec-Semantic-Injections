"""ItemTable — InfoNCE alignment between SASRec and frozen LLM item embeddings.

This is the *one* novel piece of code in ItemTable. Everything else
...
introduce gradient conflict with BCE and hurt performance. ItemTable's
...
class AlignProjector(nn.Module):
    ...
    * ``hidden_dim > 0`` — a 2-layer MLP (Linear → ReLU → Dropout →
    ...
    The projector is owned by :class:`~sasrec_injection.training.item_table_trainer.ItemTableTrainer`
    rather than the SASRec model, because:

    1. It's only used at training time — at inference, ItemTable falls
    ...
    2. Keeping it off the model means saved checkpoints have the same
       ...
       evaluation script works on ItemTable checkpoints unchanged.
