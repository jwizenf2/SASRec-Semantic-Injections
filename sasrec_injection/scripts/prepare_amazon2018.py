...
Usage:
    uv run python sasrec_injection/scripts/prepare_amazon2018.py --datasets beauty sports
    uv run python sasrec_injection/scripts/prepare_amazon2018.py --datasets beauty
"""
...
from sasrec_injection.data.amazon2018 import load_amazon2018, load_amazon2018_meta
from sasrec_injection.data.splitting import (
    generate_negative_samples,
    leave_one_out_split,
    save_negative_samples,
)
from sasrec_injection.data.loaders import build_user_sequences
...
def prepare_dataset(dataset: str, data_dir: str, model_name: str) -> None:
    ...
    emb_path = Path("sasrec_injection/outputs/embeddings") / f"{dataset}2018.pt"
    if emb_path.exists():
