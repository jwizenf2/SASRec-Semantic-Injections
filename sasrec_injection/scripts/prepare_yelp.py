"""Prepare the Yelp Open Dataset for ItemTable.
...
1. Verify ``data/yelp/`` exists and contains the two files ItemTable
...
(``train_baseline.py``, ``train_item_table.py``, ``extract_llm_embeddings.py``)
...
# Make ``import sasrec_injection...`` work when running this script from the
# project root.
sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "src"))

from sasrec_injection.data.loaders import (
    build_user_sequences,
    load_ratings,
    preprocess,
)
from sasrec_injection.data.splitting import leave_one_out_split
from sasrec_injection.data.yelp import BUSINESS_FILE, REVIEW_FILE, _yelp_dir
...
def main() -> int:
    ...
    log("       uv run python sasrec_injection/scripts/extract_llm_embeddings.py \\")
    log("           --config sasrec_injection/configs/sasrec_injection_yelp.yaml \\")
    log("           --output sasrec_injection/outputs/embeddings/yelp.pt")
    log("  2. Train P1 baseline (3 seeds):")
    log("       uv run python sasrec_injection/scripts/train_baseline.py \\")
    log("           --config sasrec_injection/configs/p1_yelp.yaml \\")
    log("           --seeds 42 7 18")
    log("  3. Train ItemTable (3 seeds):")
    log("       uv run python sasrec_injection/scripts/train_item_table.py \\")
    log("           --config sasrec_injection/configs/sasrec_injection_yelp.yaml \\")
    log("           --seeds 42 7 18 --lambdas 0.1")
    log("  4. Eval both:")
    log("       uv run python sasrec_injection/scripts/eval_fullrank.py \\")
    log("           --config sasrec_injection/configs/sasrec_injection_yelp.yaml \\")
    log("           --seeds 42 7 18")
    log("")
    return 0
