...
Given a ItemTable experiment config (which names a dataset), this
script:

1. Loads the dataset's ratings to build the item id → contiguous int
   map (so the saved tensor's row indices line up with what ItemTable
...
The saved tensor is consumed by the ItemTable training script via the
...
ItemTable uses 0.6B/1024-dim by default.
...
::
    ...
    uv run python sasrec_injection/scripts/extract_llm_embeddings.py \\
        ...
        --output sasrec_injection/outputs/embeddings/video_games.pt
"""
...
# project root or from anywhere else. Adds ``sasrec_injection/src/`` to the path.
sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "src"))

from sasrec_injection.config import load_config
from sasrec_injection.data.item_metadata import ItemMetadata
from sasrec_injection.data.loaders import is_amazon, is_yelp, load_ratings, preprocess
...
def extract_embeddings(
    ...
    Args:
        metadata: Per-item title + side-info (already keyed by remapped
            item ids so this loop's ``item_id`` matches the ItemTable
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Extract Qwen3-Embedding item embeddings for ItemTable "
    ...
    parser.add_argument(
        ...
        help="ItemTable config (used only to resolve dataset name).",
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "is ItemTable's default — small and fast on M2 Max."
        ),
    )
    parser.add_argument(
        "--output",
        ...
        help=(
            ...
            "exactly where the ItemTable trainer will look for it. Pass "
    ...
    # Resolve dataset from the ItemTable config so the row indices in
    ...
    # ItemTable config's shape assertion still passes.
