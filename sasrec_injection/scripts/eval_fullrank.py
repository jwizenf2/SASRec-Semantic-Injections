"""End-of-training evaluation of saved SASRec checkpoints (P1 or ItemTable).
...
ItemTable rows directly into either kind of comparison table.
...
* Bare state-dicts (``torch.save(model.state_dict(), path)``) — the
  P1 baseline format.
* Wrapped dicts (``{"model": state_dict}``) — the ItemTable format.
...
::
    ...
    uv run python sasrec_injection/scripts/eval_fullrank.py \\
        --config sasrec_injection/configs/p1_video_games.yaml --seeds 42

    # Evaluate ItemTable checkpoints (write a temp p1-style yaml whose
    # output_dir points at sasrec_injection/outputs/sasrec_injection_video_games/lambda_0.1
...
# Make ``import sasrec_injection...`` work whether this script is invoked from
# the project root or anywhere else.
sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "src"))

from sasrec_injection.config import SASRecConfig
from sasrec_injection.data.dataset import FullRankEvalDataset, SASRecEvalDataset
...
from sasrec_injection.evaluation.metrics import full_rank_evaluate, sampled_evaluate
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "for both P1 baseline and ItemTable checkpoints because "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Use a P1-style yaml; for ItemTable, write a small temp yaml "
    ...
    # ItemTable runs that share preprocessing get the same negatives —
