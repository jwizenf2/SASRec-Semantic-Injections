...
* ``sasrec_injection/outputs/full_rank_results.csv`` — full-catalog ranking.
* ``sasrec_injection/outputs/sampled_results.csv``   — sampled@100 (LLM-ESR
...
Usage:
    uv run python sasrec_injection/scripts/results_table.py
    uv run python sasrec_injection/scripts/results_table.py --datasets video_games
"""
...
OUTPUTS_DIR = REPO_ROOT / "sasrec_injection" / "outputs"
...
# ``sasrec_injection/outputs/<prefix><dataset>/`` runs.
METHOD_PREFIXES = {
    "p1_": "SASRec",
    "sasrec_injection_": "ItemTable",
}
BASELINE_LABEL = "SASRec"

# Ablation runs live under ``sasrec_injection/outputs/ablations/<key>/`` and
...
def discover_methods(datasets: list[str] | None = None) -> list[MethodSpec]:
    ...
    # 1. Top-level baseline / ItemTable runs: <prefix><dataset>/.
...
def build_protocol_table(long_df: pd.DataFrame, protocol: str) -> pd.DataFrame:
    ...
    for lbl in [BASELINE_LABEL, *ablation_labels, "ItemTable"]:
        if lbl not in seen:
