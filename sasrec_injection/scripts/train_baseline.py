...
This is the comparison baseline for ItemTable. Numbers from a single
...
ItemTable must beat HR@10 + 0.005 absolute (≥ 0.0634) to clear the
decision gate.
...
::
    ...
    uv run python sasrec_injection/scripts/train_baseline.py \\
        --config sasrec_injection/configs/p1_video_games.yaml --seeds 42 123 456
...
# the project root or from ``sasrec_injection/``.
...
def prepare_data(cfg: SASRecConfig) -> tuple:
    """Load ratings, run preprocess + split, generate negatives.

    Args:
        cfg: Loaded :class:`SASRecConfig`.
