"""A3 — Hidden-State InfoNCE distillation (location ablation)."""
...
from sasrec_injection.data.dataset import SASRecEvalDataset, SASRecTrainDataset, create_eval_loader, create_train_loader
...
from sasrec_injection.training.hidden_distill_trainer import HiddenStateDistillTrainer
sys.path.insert(0, str(Path(__file__).resolve().parent))
from train_item_table import log, prepare_data, load_llm_embeddings
...
def train_seed(cfg, seed, split, neg_samples, llm_item_emb, resume=False):
    ...
    val_ds = SASRecEvalDataset(split.train_seqs, split.val_targets, {}, max_seq_len=cfg.model.max_seq_len)

    from sasrec_injection.data.splitting import load_negative_samples
    neg_path = Path(cfg.output_dir) / "neg_samples.npz"
    if neg_path.exists():
        neg_samples_loaded = load_negative_samples(str(neg_path))
    else:
        neg_samples_loaded = neg_samples
    val_ds = SASRecEvalDataset(split.train_seqs, split.val_targets, neg_samples_loaded, max_seq_len=cfg.model.max_seq_len)
    ...
    test_ds = SASRecEvalDataset(test_seqs, split.test_targets, neg_samples_loaded, max_seq_len=cfg.model.max_seq_len)
    metrics = sampled_evaluate(model, create_eval_loader(test_ds), device, ks=cfg.evaluation.ks)
    log(f"\n[Seed {seed}] A3 test:"); [log(f"  {k}: {v:.4f}") for k, v in sorted(metrics.items())]
    with open(seed_dir / "results.json", "w") as f:
        json.dump({"seed": seed, "ablation": "A3_hidden_distill", "test_metrics": metrics}, f, indent=2)
    return metrics


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", default="sasrec_injection/configs/ablations/A3_hidden_distill.yaml")
    ...
    for lam in lambdas:
        ...
        log(f"A3 | seeds={cfg.seeds} | λ={lam}")
