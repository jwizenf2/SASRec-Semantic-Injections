...
weighting$\rangle$ space on Video Games under sampled@100.
The overall and head H@10 columns show that
\emph{initialization is the dominant lever}: LLM-PCA init lifts
sampled overall H@10 from $0.7194$ to $0.7522$ and head H@10
from $0.8719$ to $0.8872$, and no subsequent combination of
alignment or weighting moves either metric by more than
$\pm 0.004$. The torso and tail columns tell the complementary
story: init alone lifts sampled tail from $0.2263$ to $0.2848$
and torso from $0.5188$ to $0.5840$, but combining it with
frequency-weighted InfoNCE alignment goes furthest, reaching
sampled tail H@10 of $0.3195$ and torso H@10 of $0.5894$ (the
best of any configuration in both columns). The two ingredients
act on orthogonal axes: init dominates head accuracy,
frequency-weighted alignment dominates the sampled torso and tail
...
\begin{table}[!htbp]
  \centering
  \footnotesize
  \caption{Item-table integration decomposition on Video Games (Sampled@100).
  ...
  H@10 and head H@10 are mostly determined by initialization;
  torso and tail metrics are where alignment, and specifically
  frequency weighting on top of LLM-PCA, makes the difference.
  ...
  \begin{tabular}{lllrrrr}
    \toprule
    & & & \multicolumn{4}{c}{Sampled@100} \\
    \cmidrule(lr){4-7}
    Init & Align & Weight & Overall H@10 & Head H@10 & Torso H@10 & Tail H@10 \\
    \midrule
    Xavier  & ---     & ---     & 0.7194{\tiny$\pm$.0021} & 0.8719{\tiny$\pm$.0004} & 0.5188{\tiny$\pm$.0059} & 0.2263{\tiny$\pm$.0052} \\
    LLM-PCA & ---     & ---     & 0.7522{\tiny$\pm$.0009} & 0.8872{\tiny$\pm$.0015} & 0.5840{\tiny$\pm$.0037} & 0.2848{\tiny$\pm$.0060} \\
    Xavier  & InfoNCE & uniform & \textbf{0.7572}{\tiny$\pm$.0016} & \textbf{0.8909}{\tiny$\pm$.0006} & \underline{0.5854}{\tiny$\pm$.0038} & 0.3114{\tiny$\pm$.0031} \\
    LLM-PCA & InfoNCE & uniform & \underline{0.7556}{\tiny$\pm$.0009} & \underline{0.8900}{\tiny$\pm$.0018} & 0.5816{\tiny$\pm$.0019} & 0.3116{\tiny$\pm$.0017} \\
    Xavier  & InfoNCE & freq    & 0.7446{\tiny$\pm$.0012} & 0.8768{\tiny$\pm$.0013} & 0.5711{\tiny$\pm$.0018} & \underline{0.3153}{\tiny$\pm$.0060} \\
    \textbf{LLM-PCA} & \textbf{InfoNCE} & \textbf{freq (ours)} & 0.7539{\tiny$\pm$.0009} & 0.8836{\tiny$\pm$.0016} & \textbf{0.5894}{\tiny$\pm$.0028} & \textbf{0.3195}{\tiny$\pm$.0064} \\
    \bottomrule
  \end{tabular}}%
\end{table}
...
slightly from $0.8719$ to $0.8836$.
